...
// Замена img на SVG
  ...
  // Фиксация таблицы при скролле
/* function fixTableOnScroll() {
    // Получаем положение и размеры сайдбара после загрузки страницы
    var sidebarTop = $('.flat__body').offset().top;
    var sidebarLeft = $('.flat__body').offset().left;
    var sidebarHeight = $('.flat__body').height();

    $(window).scroll(function() {
      //получаем текущие координаты
      var top = $(document).scrollTop();
      //Если проскролили дальше, чем верхний край сайдбара был, когда страница открылась, меняем позиционирование
      if (top >= sidebarHeight) {
        $('.flat-table').addClass('affix');
      } else {
        $('.flat-table').removeClass('affix');
      }
    });
  }
